...
const API_END_POINT =
  "https://food-app-server-hk06.onrender.com/api/v1/restaurant";
    ...
    (set, get) => ({
      ...
      searchRestaurant: async (
        ...
        try {
          ...
          params.set("searchQuery", serachQuery);
      ...
      updatedMenuToRestaurant: (undatedMenu: MenuItem) => {
        set((state: any) => {
          if (state.restaurant) {
            const updatedMenuList = state.restaurant.menus.map((menu: any) =>
              menu._id === undatedMenu._id ? undatedMenu : menu
            );
            return {
              restaurant: {
                ...state.restaurant,
